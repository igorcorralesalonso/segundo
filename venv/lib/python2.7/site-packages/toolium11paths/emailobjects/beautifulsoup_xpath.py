...
from bs4 import (
    NavigableString, DEFAULT_OUTPUT_ENCODING, Tag, ProcessingInstruction, Declaration, Comment, BeautifulSoup, CData,
    PageElement
)
...
class BinaryExpr(BaseExpr):
    def __init__(self, op, left, right):
        ...
        (lneedContextPosition, rneedContextPosition) = (
        getattr(left, 'needContextPosition', None), getattr(right, 'needContextPosition', None))
        (lneedContextNode, rneedContextNode) = (
        getattr(left, 'needContextNode', None), getattr(right, 'needContextNode', None))
    ...
    def _compare(op, comp, left, right, ctx):
        ...
        if op == '=' or op == '!=':
            if typeof(left) == 'boolean' or typeof(right) == 'boolean':
                return comp(to_boolean(left), to_boolean(right))
...
class PathExpr(BaseExpr):
    ...
    def evaluate(self, ctx):
        ...
        if not getattr(nodeset, 'isNodeSet', None):
            raise exceptions.BaseException('Filter nodeset must be nodeset type')
    ...
    def parse(cls, lexer):
        ...
        if ops.get(lexer.peek()):
            ...
            if op == '/' and lexer.empty() or (
                                token != '.' and token != '..' and token != '@' and token != '*' and not re_has_ualpha.search(
                    token)):
        ...
        if not special and getattr(self, 'op', None) == '//':
            if not self.needContextPosition and self.axis == 'child':
                if getattr(self, 'quickAttr', None):
                    ...
                    nodeset = NodeUtil.getDescendantNodes(self.test, node, NodeSet(), self.attrName, attrValue,
                                                          prevNodeset, prevIndex)
                    nodeset = self.evaluatePredicates(nodeset, 1)
                else:
                    nodeset = NodeUtil.getDescendantNodes(self.test, node, NodeSet(), None, None, prevNodeset,
                                                          prevIndex)
...
class NodeSet(object):
    ...
    def bool(self):
        return to_boolean(self.length or self.only)
...
class XPathResult(object):
    ...
    def __init__(self, value, type):
        ...
        if type == self.NUMBER_TYPE:
            ...
            self.booleanValue = value.bool() if getattr(value, 'isNodeSet', None) else to_boolean(value)
...
class XPathExpression(object):
    ...
    def evaluate(self, node):
        return self.expr.evaluate(Ctx(node)).elem()

    def evaluate_list(self, node):
        return self.expr.evaluate(Ctx(node)).elems() or []
...
class BSXPathEvaluator(BeautifulSoup):
    ...
    def evaluate(self, expr):
        return XPathExpression(expr).evaluate(self)

    def evaluate_list(self, expr):
        return XPathExpression(expr).evaluate_list(self)
